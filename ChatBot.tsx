...
Sprich den Nutzer mit "Du" an. Sei professionell aber nahbar.
...
const ChatBot: React.FC = () => {
    ...
    const [messages, setMessages] = useState<Message[]>([
        { role: 'model', text: 'Willkommen bei titanspear.ai. Ich bin dein AI-Consultant. Welchen Prozess möchtest du automatisieren?' }
    ]);
    ...
    const handleSend = async () => {
        ...
        try {
            // Fix: Initialize GoogleGenAI with process.env.API_KEY directly before use
            const ai = new GoogleGenAI({ apiKey: process.env.API_KEY });
            
            // Fix: Use 'gemini-3-flash-preview' for text tasks as recommended
            ...
            // Fix: Access .text property directly (not a method) from the response
            ...
            if (error.message && (error.message.includes("Key") || error.message.includes("API"))) {
                errMsg = "System-Fehler: API Verbindung fehlgeschlagen.";
            }
                    ...
                    >
                            ...
                            <div className="flex items-center gap-3">
                                ...
                                <div>
                                    ...
                                    <p className="text-cyan-500 text-[10px] font-mono uppercase tracking-wider flex items-center gap-1 opacity-80">
                                        <Sparkles size={8} /> Powered by Gemini 3
                                    </p>
                                </div>
                            </div>
                                ...
                                >
                                    ...
                                    <div 
                                        className={`max-w-[85%] p-4 text-sm leading-relaxed shadow-lg backdrop-blur-md border transition-all duration-300 ${
                                            ...
                                            : 'bg-slate-800/60 text-slate-200 rounded-2xl rounded-bl-sm border-white/10 hover:border-cyan-500/40 hover:shadow-[0_0_15px_rgba(6,182,212,0.1)] hover:border-cyan-500/40 hover:shadow-[0_0_15px_rgba(6,182,212,0.1)] hover:bg-slate-800/80 hover:text-white'
                                        }`}
                                    >
                                        {msg.text}
                                    </div>
                        ...
                        <div className="p-4 border-t border-white/10 bg-black/60 backdrop-blur-md">
                            ...
                            <div className="text-[10px] text-center text-slate-600 mt-2 font-medium">
                                titanspear.ai Bot kann Fehler machen. Überprüfe wichtige Infos.
                            </div>
                        </div>
                    </motion.div>
                )}
